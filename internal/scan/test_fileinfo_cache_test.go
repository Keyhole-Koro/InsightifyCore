...
func TestGetSizeAndPreviewCache(t *testing.T) {
	...
	if err != nil || sz1 <= 0 {
		t.Fatalf("GetSize1: %v, sz=%d", err, sz1)
	}
	...
	if err != nil || s5 != "hello" {
		t.Fatalf("preview5: %q err=%v", s5, err)
	}
	s3, err := GetPreview(dir, "a.txt", 3)
	if err != nil || s3 != "hel" {
		t.Fatalf("preview3: %q err=%v", s3, err)
	}
	s8, err := GetPreview(dir, "a.txt", 8)
	if err != nil || s8 != "hello wo" {
		t.Fatalf("preview8: %q err=%v", s8, err)
	}
	...
	if err != nil || sz3 <= sz1 {
		t.Fatalf("GetSize3: %v sz3=%d sz1=%d", err, sz3, sz1)
	}
}
