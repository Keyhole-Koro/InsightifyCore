...
func ScanWithOptions(root string, opts Options, cb VisitFunc) error {
	...
	if opts.CacheSubtrees {
		...
		return walkSubtreeCached(rClean, ".", 0, opts.MaxDepth, ig, igKey, cb, opts.BypassCache)
	}

	// Fallback: full re-scan without caching
	return walkSubtreeCached(rClean, ".", 0, opts.MaxDepth, normalizeIgnores(opts.IgnoreDirs), "", cb, true)
}
...
func walkSubtreeCached(root string, relPrefix string, depth int, maxDepth int, ignores []string, ignoreKey string, cb VisitFunc, bypass bool) error {
	...
	for _, e := range entries {
		...
		if e.IsDir() {
			...
			if err := walkSubtreeCached(root, childRel, depth+1, maxDepth, ignores, ignoreKey, cb, bypass); err != nil {
				return err
			}
		} else {
			var size int64
