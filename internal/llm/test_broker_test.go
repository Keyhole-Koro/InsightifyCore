...
type fakeLimiter struct{ calls int32; failAt int32 }
func (f *fakeLimiter) Acquire(ctx context.Context) error {
    n := atomic.AddInt32(&f.calls, 1)
    if f.failAt > 0 && n == f.failAt { return errors.New("boom") }
    return nil
}
...
func TestBrokerReserveSuccess(t *testing.T) {
    ...
    for i := 0; i < 3; i++ { tester.True(t, TakeCredit(ctx), "credit available") }
...
func TestBrokerReserveError(t *testing.T) {
    ...
    if err == nil { t.Fatalf("expected error on reservation") }
