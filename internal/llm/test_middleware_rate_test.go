...
type fakeClient struct{ name string; calls int }
...
func TestRateLimitPrefersCredits(t *testing.T) {
    ...
    if _, err := cli.GenerateJSON(ctx, "p", map[string]any{}); err != nil { t.Fatal(err) }

    // Now simulate lack of credits: still should proceed, but through disabled limiter.
    if _, err := cli.GenerateJSON(context.Background(), "p", map[string]any{}); err != nil { t.Fatal(err) }
