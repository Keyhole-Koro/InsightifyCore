...
type Workspace struct {
	...
	// ActiveTabID holds the value of the "active_tab_id" field.
...
func (e WorkspaceEdges) TabsOrErr() ([]*WorkspaceTab, error) {
	if e.loadedTypes[0] {
		return e.Tabs, nil
	}
...
func (*Workspace) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case workspace.FieldID, workspace.FieldProjectID, workspace.FieldName, workspace.FieldActiveTabID:
...
func (_m *Workspace) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case workspace.FieldActiveTabID:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field active_tab_id", values[i])
			} else if value.Valid {
				_m.ActiveTabID = value.String
			}
...
// QueryTabs queries the "tabs" edge of the Workspace entity.
func (_m *Workspace) QueryTabs() *WorkspaceTabQuery {
	return NewWorkspaceClient(_m.config).QueryTabs(_m)
}
...
func (_m *Workspace) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("active_tab_id=")
	builder.WriteString(_m.ActiveTabID)
	builder.WriteString(", ")
