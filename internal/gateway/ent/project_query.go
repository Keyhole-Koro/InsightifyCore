...
func (_q *ProjectQuery) loadArtifacts(ctx context.Context, query *ArtifactQuery, nodes []*Project, init func(*Project), assign func(*Project, *Artifact)) error {
	...
	for _, n := range neighbors {
		fk := n.project_artifacts
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_artifacts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_artifacts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
