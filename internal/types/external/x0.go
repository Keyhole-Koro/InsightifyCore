...
type X0In struct {
	...
	Roots               ml.M0Out            `json:"roots"`
	Architecture        ml.M1Out            `json:"architecture"`
	ConfigSamples       []t.OpenedFile      `json:"config_samples"`
...
type IdentifierSummary struct {
	...
	Scope    cb.IdentifierScope         `json:"scope"`
	Requires []cb.IdentifierRequirement `json:"requires,omitempty"`
...
type ExternalSystem struct {
	...
	Evidence    []t.EvidenceRef `json:"evidence,omitempty"`
	Confidence  float64         `json:"confidence"`
}
